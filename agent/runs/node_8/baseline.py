...
默认 run_model 走新的 BPR pairwise 训练流程（见 run_fm_bpr），支持 Dynamic Negative
Sampling (DNS)：每个正样本从同用户负样本池中抽 neg_pool 个候选，用当前模型打分，
保留分数最高（最难）的一个做负样本，比均匀采样更贴近 GAUC/nDCG 这类 top-heavy 指标。
...
def run_fm_bpr(splits, k=16, lr=0.001, epochs=40, bs=8192, patience=4, neg_k=1,
               neg_pool=8, seed=0, verbose=True):
    """BPR pairwise-ranking training loop with Dynamic Negative Sampling (DNS).
    For each positive training row, draw `neg_pool` candidate negatives from the
    SAME user's negative rows (vectorized, no per-row python loop), score all of
    them with the current model's forward pass, and keep the highest-scoring
    ("hardest") one for the pairwise update -- neg_pool=1 degenerates to the
    original uniform-random single-negative sampling. Falls back to a global
    random negative for users with no negative rows in train."""
    enc, dim = encode(splits)
    Xtr, ytr, _ = enc['train']; Xva, yva, uva = enc['valid']; Xte, yte, ute = enc['test']
    ...
    rng = np.random.default_rng(seed)
    best, best_state, bad = -1, None, 0
    N = max(1, int(neg_pool))

    def sample_negs_pool(pos_idx_batch, n, rng):
        """Return (B, n) row indices: n same-user negative candidates per positive."""
        B = len(pos_idx_batch)
        u = user_col[pos_idx_batch]
        u_clamped = np.clip(u, 0, max_user - 1)
        cnt = neg_count[u_clamped]
        has_neg = cnt > 0
        offset = rng.integers(0, np.maximum(cnt, 1), size=(n, B))         # (n,B)
        neg_pos = neg_start[u_clamped][None, :] + offset
        neg_pos = np.clip(neg_pos, 0, n_neg_total - 1)
        chosen = neg_row_idx_sorted[neg_pos]                              # (n,B)
        n_missing = int((~has_neg).sum())
        if n_missing > 0:
            chosen = chosen.copy()
            miss_idx = np.where(~has_neg)[0]
            chosen[:, miss_idx] = neg_row_idx_sorted[
                rng.integers(0, n_neg_total, size=(n, n_missing))]
        return chosen.T                                                   # (B,n)

    def pick_hard_negs(pos_batch, rng):
        cand = sample_negs_pool(pos_batch, N, rng)          # (B,N)
        if N == 1:
            return cand[:, 0]
        B = cand.shape[0]
        flat = cand.reshape(-1)
        zc, _, _ = m.logits(Xtr[flat])
        zc = zc.reshape(B, N)
        best_j = np.argmax(zc, axis=1)
        return cand[np.arange(B), best_j]
    ...
    for ep in range(1, epochs + 1):
        ...
        for i in range(0, len(order_pos), bs):
            ...
            for _ in range(max(1, neg_k)):
                neg_batch = pick_hard_negs(pos_batch, rng)
...
def run_model(splits, hparams, seed=0, verbose=True):
    ...
    of model family. Default: BPR pairwise-ranking FM training with Dynamic
    Negative Sampling (see run_fm_bpr)."""
    kwargs = dict(hparams)
    kwargs.pop('bpr', None)
...
if __name__ == '__main__':
    ...
    ap.add_argument('--neg_pool', type=int, default=8)
    ...
    res = {'pop': run_pop, 'random': lambda s: run_random(s, a.seed),
           ...
           'bpr': lambda s: run_fm_bpr(s, k=a.k, lr=a.lr, epochs=a.epochs,
                                        neg_pool=a.neg_pool, seed=a.seed)}[a.model](splits)
    print(f"\n=== {a.model} (seed={a.seed}) ===")
