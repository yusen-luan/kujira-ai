...
LABEL = 'long_view'
AUX_LABEL = 'is_like'  # auxiliary multi-task training signal only (never used as an input feature)
...
def load(data_dir):
    ...
    Each row: (date, user_id, video_id, author_id, tab, duration_ms, long_view_label, is_like_label).
    is_like_label is carried through purely as an auxiliary TRAINING TARGET for multi-task
    learning (never fed into encode()'s feature construction) -- not a leaked input feature."""
    vid2author = {}
