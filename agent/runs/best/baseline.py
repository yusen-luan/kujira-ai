...
def run_fm_bpr(splits, k=16, lr=0.001, epochs=40, bs=8192, patience=4, neg_k=1, seed=0, verbose=True,
    ...
    if _capture is not None:
        _capture['model'] = m  # opt-in side channel for submission generation only --
                                # never populated unless a caller explicitly passes a dict,
                                # so the {'valid':..., 'test':...} contract run_and_report.py
                                # relies on is unchanged for the orchestrator's own calls.
    return {'valid': evaluate(uva, yva, m.predict(Xva)),
            'test':  evaluate(ute, yte, m.predict(Xte))}


def run_model(splits, hparams, seed=0, verbose=True, _capture=None):
    ...
    kwargs = dict(hparams)
    kwargs.pop('bpr', None)
    return run_fm_bpr(splits, seed=seed, verbose=verbose, _capture=_capture, **kwargs)
