"""KuaiRand-Pure baselines。
  ...
  --model bpr   : BPR pairwise 单任务训练（tag-augmented fields）
  --model mt    : BPR + 共享嵌入的辅助任务 (is_like) 联合训练
  --model random: 随机打分（下界，用来自检评测代码没坏）
默认 run_model 走 ensemble：单任务 BPR-FM（含 tag 特征） + 多任务 FM_MT（辅助 is_like 头），
两者 z-score 后平均预测分数。只依赖 numpy。用法见 README.md
"""
...
# ---------------- Factorization Machine (single-task) ----------------
class FM:
...
class FM_MT:
    """Shared embedding table V feeds two heads: a main head (long_view, BPR pairwise) and
    an auxiliary head (is_like, pointwise BCE, weighted by alpha), both reusing the same
    quadratic interaction term from V so the auxiliary loss regularizes shared embeddings."""
    ...
    def step_bpr_mt(self, Xpos, Xneg, aux_pos, aux_neg, alpha=0.03):
        B = len(Xpos)
...
def _build_neg_sampler(Xtr, ytr):
...
def _fit_single_task(enc, dim, k=16, lr=0.001, epochs=40, bs=8192, patience=4, neg_k=1,
                      seed=0, verbose=True, tag='A'):
...
def _fit_multi_task(enc, dim, k=16, lr=0.001, epochs=40, bs=8192, patience=4, neg_k=1,
                     alpha_aux=0.03, seed=0, verbose=True, tag='B'):
    Xtr, ytr, _ = enc['train']; Xva, yva, uva = enc['valid']
...
def run_fm_bpr(splits, k=16, lr=0.001, epochs=40, bs=8192, patience=4, neg_k=1, seed=0, verbose=True):
    """Single-task BPR-FM (tag-augmented fields), kept for reference / --model bpr CLI use."""
    enc, dim = encode(splits)
    Xva, yva, uva = enc['valid']; Xte, yte, ute = enc['test']
    m = _fit_single_task(enc, dim, k=k, lr=lr, epochs=epochs, bs=bs, patience=patience,
                          neg_k=neg_k, seed=seed, verbose=verbose, tag='bpr')
    return {'valid': evaluate(uva, yva, m.predict(Xva)),
            'test':  evaluate(ute, yte, m.predict(Xte))}


def run_fm_bpr_mt(splits, k=16, lr=0.001, epochs=40, bs=8192, patience=4, neg_k=1,
                   alpha_aux=0.03, seed=0, verbose=True):
    """Multi-task BPR-FM with auxiliary is_like head, kept for reference / --model mt CLI use."""
    enc, dim = encode(splits)
    Xva, yva, uva = enc['valid']; Xte, yte, ute = enc['test']
    m = _fit_multi_task(enc, dim, k=k, lr=lr, epochs=epochs, bs=bs, patience=patience,
                         neg_k=neg_k, alpha_aux=alpha_aux, seed=seed, verbose=verbose, tag='mt')
    return {'valid': evaluate(uva, yva, m.predict(Xva)),
            'test':  evaluate(ute, yte, m.predict(Xte))}
...
def run_model(splits, hparams, seed=0, verbose=True):
    """Ensemble two structurally different mechanisms in one call:
    (A) single-task BPR-FM on tag-augmented fields (feature_addition lever, iter6)
    (B) multi-task BPR-FM with an auxiliary is_like head (multi_task lever, iter14)
    Predictions from both are z-scored per split and averaged, since the two models'
    errors stem from different sources (extra input feature vs. extra supervision
    signal) and should be more decorrelated than two same-mechanism variants."""
    kwargs = dict(hparams)
    kwargs.pop('bpr', None)
    alpha_aux = kwargs.pop('alpha_aux', 0.03)
    k = kwargs.get('k', 16); lr = kwargs.get('lr', 0.001)
    epochs = kwargs.get('epochs', 40); bs = kwargs.get('bs', 8192)
    patience = kwargs.get('patience', 4); neg_k = kwargs.get('neg_k', 1)

    enc, dim = encode(splits)
    Xva, yva, uva = enc['valid']; Xte, yte, ute = enc['test']

    m_a = _fit_single_task(enc, dim, k=k, lr=lr, epochs=epochs, bs=bs, patience=patience,
                            neg_k=neg_k, seed=seed, verbose=verbose, tag='A-bpr+tag')
    m_b = _fit_multi_task(enc, dim, k=k, lr=lr, epochs=epochs, bs=bs, patience=patience,
                           neg_k=neg_k, alpha_aux=alpha_aux, seed=seed, verbose=verbose, tag='B-mt')

    va_a, va_b = m_a.predict(Xva), m_b.predict(Xva)
    te_a, te_b = m_a.predict(Xte), m_b.predict(Xte)

    ens_va = 0.5 * (_zscore(va_a) + _zscore(va_b))
    ens_te = 0.5 * (_zscore(te_a) + _zscore(te_b))

    return {'valid': evaluate(uva, yva, ens_va),
            'test':  evaluate(ute, yte, ens_te)}

if __name__ == '__main__':
    ap = argparse.ArgumentParser()
    ap.add_argument('--data_dir', default='./KuaiRand-Pure/data',
                    help='KuaiRand-Pure 解压后的 data 目录')
    ap.add_argument('--model', default='ensemble', choices=['pop', 'fm', 'bpr', 'mt', 'random', 'ensemble'])
    ap.add_argument('--k', type=int, default=16)
    ap.add_argument('--lr', type=float, default=0.001)
    ap.add_argument('--epochs', type=int, default=40)
    ap.add_argument('--alpha_aux', type=float, default=0.03)
    ...
    res = {'pop': run_pop, 'random': lambda s: run_random(s, a.seed),
           ...
           'mt': lambda s: run_fm_bpr_mt(s, k=a.k, lr=a.lr, epochs=a.epochs,
                                          alpha_aux=a.alpha_aux, seed=a.seed),
           'ensemble': lambda s: run_model(s, {'k': a.k, 'lr': a.lr, 'epochs': a.epochs,
