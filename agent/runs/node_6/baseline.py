"""KuaiRand-Pure baselines。
  ...
  --model fm    : Factorization Machine，pairwise BPR loss（同用户内 正/负样本对比），UBPR 风格
                  popularity-propensity reweighting on the positive side
...
# ---------------- Factorization Machine (pairwise BPR training, UBPR-style reweighting) ----------------
class FM:
    ...
    def bpr_step(self, Xpos, Xneg, w=None):
        """Pairwise BPR update: push score(pos) above score(neg) for same-user pairs.
        w: optional per-pair weight (UBPR-style inverse-propensity reweighting of the
        positive side), shape (B,). If None, behaves exactly like plain BPR."""
        B = len(Xpos)
        ...
        g = ((p - 1.0) * w / B).astype(np.float32)   # dL/dzpos = g ; dL/dzneg = -g
        ...
        return float(-np.mean(w * np.log(p + 1e-9)))
...
def run_fm(splits, k=16, lr=0.001, epochs=40, bs=8192, patience=4, seed=0, verbose=True,
           neg_per_pos=1, pop_theta=0.5, pop_weight_clip=20.0):
    ...
    # --- UBPR-style popularity-propensity weight per encoded video id, from train
    # impression counts (Saito 2020: propensity(v) = (count(v)/max_count)^theta,
    # positive-side loss weight = 1/propensity, clipped to bound variance). theta=0
    # recovers plain (unweighted) BPR.
    vid_col = FIELDS.index('video_id')
    pop_count = np.bincount(Xtr[:, vid_col], minlength=dim).astype(np.float64)
    max_count = max(pop_count.max(), 1.0)
    propensity = np.power(pop_count / max_count, pop_theta)
    min_prop = 1.0 / max(pop_weight_clip, 1.0)
    propensity = np.clip(propensity, min_prop, 1.0)
    item_weight = (1.0 / propensity).astype(np.float32)

    best, best_state, bad = -1, None, 0
    for ep in range(1, epochs + 1):
        t0 = time.time()
        # resample a fresh negative per positive each epoch
        ...
        for i in range(0, n_pairs, bs):
            ...
            w = item_weight[Xpos[:, vid_col]]
            losses.append(m.bpr_step(Xpos, Xneg, w=w))
...
if __name__ == '__main__':
    ...
    ap.add_argument('--pop_theta', type=float, default=0.5)
    ap.add_argument('--pop_weight_clip', type=float, default=20.0)
    a = ap.parse_args()
    ...
    res = {'pop': run_pop, 'random': lambda s: run_random(s, a.seed),
           'fm': lambda s: run_fm(s, k=a.k, lr=a.lr, epochs=a.epochs, seed=a.seed,
                                   neg_per_pos=a.neg_per_pos, pop_theta=a.pop_theta,
                                   pop_weight_clip=a.pop_weight_clip)}[a.model](splits)
    print(f"\n=== {a.model} (seed={a.seed}) ===")
