...
class FM:
    ...
    # ---- BPR pairwise step (optionally per-pair-weighted, e.g. for popularity debiasing) ----
    def step_bpr(self, Xpos, Xneg, w=None):
        B = len(Xpos)
        ...
        if w is not None:
            base = base * w
        coef = base.astype(np.float32)         # dL/dz_pos (weighted)
        gV = np.zeros_like(self.V); gW = np.zeros_like(self.W)
        ...
        if w is not None:
            return float(np.average(-np.log(p + 1e-9), weights=w))
...
def run_fm_bpr(splits, k=16, lr=0.001, epochs=40, bs=8192, patience=4, neg_k=1,
                pop_alpha=0.0, seed=0, verbose=True):
    ...
    pop_alpha > 0 additionally reweights each training pair by
    (video popularity in train)^(-pop_alpha), mean-normalized over positives so that
    pop_alpha == 0 exactly reproduces the unweighted BPR baseline. This down-weights
    the gradient contribution of positives on ultra-popular videos, per the
    popularity-bias literature's "reweight inversely to item popularity" debiasing
    recipe, to counter the EDA's documented Gini=0.79 video popularity skew.
    """
    enc, dim = encode(splits)
    Xtr, ytr, _ = enc['train']; Xva, yva, uva = enc['valid']; Xte, yte, ute = enc['test']
    ytr_i = ytr.astype(np.int64)
    user_col = Xtr[:, 0].astype(np.int64)                 # field 0 is user_id, offset 0
    video_col = Xtr[:, 1].astype(np.int64)                # field 1 is video_id, offset 1
    ...
    # --- popularity-debiasing weight, precomputed once over the full train set ---
    vid_counts = np.bincount(video_col, minlength=dim).astype(np.float64)
    vid_counts = np.maximum(vid_counts, 1.0)
    row_pop = vid_counts[video_col]                       # per-row video popularity
    if pop_alpha > 0:
        raw_w = row_pop ** (-pop_alpha)
        norm = raw_w[pos_row_idx].mean()
        pair_weight_all = (raw_w / norm).astype(np.float32)
    else:
        pair_weight_all = None
    ...
    for ep in range(1, epochs + 1):
        ...
        for i in range(0, len(order_pos), bs):
            ...
            w_batch = pair_weight_all[pos_batch] if pair_weight_all is not None else None
            for _ in range(max(1, neg_k)):
                if neg_k > 1:
                    neg_batch = sample_negs(pos_batch, rng)
                losses.append(m.step_bpr(Xtr[pos_batch], Xtr[neg_batch], w=w_batch))
        va = evaluate(uva, yva, m.predict(Xva))
        if verbose:
            print(f"  epoch {ep:2d} | loss {np.mean(losses):.4f} | valid GAUC {va['GAUC']:.4f} "
                  f"nDCG@5 {va['nDCG@5']:.4f} primary {va['primary']:.4f} | {time.time()-t0:.1f}s")
