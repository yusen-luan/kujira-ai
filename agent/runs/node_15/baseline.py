...
def run_model(splits, hparams, seed=0, verbose=True):
    ...
    of model family. Reads only the keys it needs via hparams.get(name, default) so
    it never raises on an unexpected extra key and never assumes a key is present
    (per the harness contract) -- the previous **hparams-unpacking version would
    raise TypeError if hparams ever carried a key run_fm doesn't accept."""
    k = int(hparams.get('k', 16))
    lr = float(hparams.get('lr', 0.001))
    epochs = int(hparams.get('epochs', 40))
    bs = int(hparams.get('bs', 8192))
    patience = int(hparams.get('patience', 4))
    neg_per_pos = hparams.get('neg_per_pos', 1)
    return run_fm(splits, k=k, lr=lr, epochs=epochs, bs=bs, patience=patience,
                  neg_per_pos=neg_per_pos, seed=seed, verbose=verbose)
...
if __name__ == '__main__':
    ...
    res = {'pop': run_pop, 'random': lambda s: run_random(s, a.seed),
           'fm': lambda s: run_fm(s, k=a.k, lr=a.lr, epochs=a.epochs, seed=a.seed,
                                   neg_per_pos=a.neg_per_pos)}[a.model](splits)
    print(f"\n=== {a.model} (seed={a.seed}) ===")
