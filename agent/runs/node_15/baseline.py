...
class FM:
    ...
    # ---- BPR pairwise step (optionally per-example weighted) ----
...
def _recency_weights(splits, decay):
    """Per-training-row weight: exp(decay * normalized_date), mean-normalized to 1.
    decay=0 -> all weights exactly 1.0 (identical to unweighted training)."""
    dates = np.array([x[0] for x in splits['train']], dtype=np.float64)
    if decay == 0 or len(dates) == 0:
        return np.ones(len(dates), dtype=np.float32)
    dmin, dmax = dates.min(), dates.max()
    span = max(1.0, dmax - dmin)
    norm = (dates - dmin) / span
    w = np.exp(decay * norm)
    w = w / w.mean()
...
def run_fm_bpr(splits, k=16, lr=0.001, epochs=40, bs=8192, patience=4, neg_k=1,
               recency_decay=0.0, seed=0, verbose=True):
    ...
    Positive examples are additionally weighted by training-date recency (recency_decay>0
    up-weights later training dates, to counter the documented train->eval distribution
    drift); recency_decay=0 exactly reproduces the unweighted baseline."""
    enc, dim = encode(splits)
    Xtr, ytr, _ = enc['train']; Xva, yva, uva = enc['valid']; Xte, yte, ute = enc['test']
    ytr_i = ytr.astype(np.int64)
    user_col = Xtr[:, 0].astype(np.int64)                 # field 0 is user_id, offset 0

    w_all = _recency_weights(splits, recency_decay)       # (len(ytr),) aligned with Xtr rows
    ...
    for ep in range(1, epochs + 1):
        ...
        for i in range(0, len(order_pos), bs):
            ...
            w_batch = w_all[pos_batch]
            for _ in range(max(1, neg_k)):
                if neg_k > 1:
                    neg_batch = sample_negs(pos_batch, rng)
                losses.append(m.step_bpr(Xtr[pos_batch], Xtr[neg_batch], w_batch))
        va = evaluate(uva, yva, m.predict(Xva))
        if verbose:
            print(f"  epoch {ep:2d} | loss {np.mean(losses):.4f} | valid GAUC {va['GAUC']:.4f} "
                  f"nDCG@5 {va['nDCG@5']:.4f} primary {va['primary']:.4f} | {time.time()-t0:.1f}s")
