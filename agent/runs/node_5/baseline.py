...
def run_fm_bpr(splits, k=16, lr=0.001, l2=1e-6, epochs=40, bs=8192, patience=4, neg_k=1, seed=0, verbose=True):
    ...
    l2 controls Adam weight-decay strength on V/W (tunable to combat post-peak overfitting
    seen in the training curve at the default near-zero 1e-6)."""
    enc, dim = encode(splits)
    Xtr, ytr, _ = enc['train']; Xva, yva, uva = enc['valid']; Xte, yte, ute = enc['test']
...
if __name__ == '__main__':
    ...
    res = {'pop': run_pop, 'random': lambda s: run_random(s, a.seed),
           ...
           'bpr': lambda s: run_fm_bpr(s, k=a.k, lr=a.lr, l2=a.l2, epochs=a.epochs, seed=a.seed)}[a.model](splits)
    print(f"\n=== {a.model} (seed={a.seed}) ===")
