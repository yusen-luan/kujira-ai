"""KuaiRand-Pure baselines。
  ...
  --model ens   : bpr 与 mt 两个已验证过的独立模型，标准化后取平均分（默认，见 run_ensemble）
...
def _train_fm_bpr(enc, dim, k, lr, epochs, bs, patience, neg_k, seed, verbose, tag):
    """Trains the single-task BPR FM in-process (shared logic behind run_fm_bpr / the
    ensemble), returning the fitted model (best-checkpoint weights already restored)."""
...
def _train_fm_bpr_mt(enc, dim, k, lr, epochs, bs, patience, neg_k, alpha_aux, seed, verbose, tag):
    """Trains the multi-task BPR+is_like FM in-process (shared logic behind run_fm_bpr_mt /
    the ensemble), returning the fitted model (best-checkpoint weights already restored)."""
    Xtr, ytr, _ = enc['train']; Xva, yva, uva = enc['valid']
...
def run_fm_bpr(splits, k=16, lr=0.001, epochs=40, bs=8192, patience=4, neg_k=1, seed=0, verbose=True):
    ...
    m = _train_fm_bpr(enc, dim, k, lr, epochs, bs, patience, neg_k, seed, verbose, tag='bpr')
    return {'valid': evaluate(uva, yva, m.predict(Xva)),
            'test':  evaluate(ute, yte, m.predict(Xte))}
...
def run_fm_bpr_mt(splits, k=16, lr=0.001, epochs=40, bs=8192, patience=4, neg_k=1,
    ...
    enc, dim = encode(splits)
    Xva, yva, uva = enc['valid']; Xte, yte, ute = enc['test']
    m = _train_fm_bpr_mt(enc, dim, k, lr, epochs, bs, patience, neg_k, alpha_aux, seed, verbose, tag='mt')
    return {'valid': evaluate(uva, yva, m.predict(Xva)),
            'test':  evaluate(ute, yte, m.predict(Xte))}
...
def run_ensemble(splits, k=16, lr=0.001, epochs=40, bs=8192, patience=4, neg_k=1,
                  alpha_aux=0.03, ens_weight=0.5, seed=0, verbose=True):
    """Trains two already-independently-validated models — the single-task BPR FM
    (current best, iter 4) and the shared-embedding multi-task BPR+is_like FM (iter 14,
    best individually-scoring rejected candidate) — and averages their z-score-normalized
    prediction scores. Both trainings use the SAME hparams/data/seed as their own best
    standalone runs; only the final combination step is new."""
    enc, dim = encode(splits)
    Xva, yva, uva = enc['valid']; Xte, yte, ute = enc['test']

    m1 = _train_fm_bpr(enc, dim, k, lr, epochs, bs, patience, neg_k, seed, verbose, tag='bpr')
    m2 = _train_fm_bpr_mt(enc, dim, k, lr, epochs, bs, patience, neg_k, alpha_aux, seed, verbose, tag='mt')

    Xva_full, _, _ = enc['valid']; Xte_full, _, _ = enc['test']
    s1_va, s2_va = m1.predict(Xva_full), m2.predict(Xva_full)
    s1_te, s2_te = m1.predict(Xte_full), m2.predict(Xte_full)

    w = float(ens_weight)
    va_score = w * _zscore(s1_va) + (1 - w) * _zscore(s2_va)
    te_score = w * _zscore(s1_te) + (1 - w) * _zscore(s2_te)

    return {'valid': evaluate(uva, yva, va_score),
            'test':  evaluate(ute, yte, te_score)}


def run_model(splits, hparams, seed=0, verbose=True):
    """Generic entrypoint the harness (run_and_report.py) always calls, regardless
    of model family. Default: ensemble of BPR-FM + multi-task BPR+is_like FM (run_ensemble)."""
    kwargs = dict(hparams)
    kwargs.pop('bpr', None)
    return run_ensemble(splits, seed=seed, verbose=verbose, **kwargs)

if __name__ == '__main__':
    ap = argparse.ArgumentParser()
    ap.add_argument('--data_dir', default='./KuaiRand-Pure/data',
                    help='KuaiRand-Pure 解压后的 data 目录')
    ap.add_argument('--model', default='ens', choices=['pop', 'fm', 'bpr', 'mt', 'ens', 'random'])
    ap.add_argument('--k', type=int, default=16)
    ap.add_argument('--lr', type=float, default=0.001)
    ap.add_argument('--epochs', type=int, default=40)
    ap.add_argument('--alpha_aux', type=float, default=0.03)
    ap.add_argument('--ens_weight', type=float, default=0.5)
    ...
    res = {'pop': run_pop, 'random': lambda s: run_random(s, a.seed),
           ...
           'ens': lambda s: run_ensemble(s, k=a.k, lr=a.lr, epochs=a.epochs,
                                          alpha_aux=a.alpha_aux, ens_weight=a.ens_weight,
                                          seed=a.seed)}[a.model](splits)
    print(f"\n=== {a.model} (seed={a.seed}) ===")
